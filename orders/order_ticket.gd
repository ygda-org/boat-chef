...
@onready var label_purple = $Panel/GridContainer/LabelPurple
...
func _ready():
	...
	label_white.text = str(order_resource.fruit_requirements[1])
	label_red.text = str(order_resource.fruit_requirements[2])
	label_purple.text = str(order_resource.fruit_requirements[3])
